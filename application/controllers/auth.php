...
class Auth extends CI_Controller {
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				echo $soap_session = $this->soapLoginClient($user->username);
				if ($soap_session =='OK'){
	...
	function logout()
	{
		...
		//log the user out
		$user = $this->data['user'] = $this->ion_auth->user($this->session->userdata('user_id'))->row();
		$this->soapLogoutClient($user->username);
	...
	function soapLoginClient($username)
	{
		try
		{
			$sClient = new SoapClient(null, array('location'	 =>
				  'http://91.196.5.252:8080/web2fax/start', 'uri'		 =>
				  'http://webserice.webfax.sip64.ru/', 'trace'		 => 1));
			$oReturn =  $sClient ->__soapCall('OpenSession', array(new SoapParam($username, 'Username')));
	...
	function soapLogoutClient($username)
	{
		try
		{
			$sClient = new SoapClient(null, array('location'	 =>
				  'http://91.196.5.252:8080/web2fax/start', 'uri' =>
				  'http://webserice.webfax.sip64.ru/', 'trace'		 => 1));
			$sClient ->__soapCall('CloseSession', array(new SoapParam($username, 'Username')));
